...
def make_new_uv(obj, new_uv_name, pack_method='PACK', margin=0.002, rotate=False):
    ...
    def execute(self, context):
        ...
        for obj in sel:
            ...
            if uv_name not in obj.data.uv_layers:
                self.report({'WARNING'}, f"{obj.name}: UV layer '{uv_name}' not found. Please run Repack UV first.")
                continue
            ...
            if not img_base and not img_orm and not img_norm:
                self.report({'INFO'}, f"{obj.name}: no data to bake — skipped.")
                continue
            ...
            if img_base:
                ...
                if src_img:
                    ...
                    try:
                        ...
                        self.report({'WARNING'}, f"{obj.name}: BaseColor bake error: {e}")
                        if s.ar_debug:
                            log(e)
            ...
            if img_orm:
                ...
                if orm_img:
                    ...
                    try:
                        ...
                        self.report({'WARNING'}, f"{obj.name}: ORM R bake error: {e}")
                    # G
                    nt.links.new(sep.outputs['G'], emis.inputs['Color'])
                    try:
                        bpy.ops.object.bake(type='EMIT', margin=8)
                    except Exception as e:
                        self.report({'WARNING'}, f"{obj.name}: ORM G bake error: {e}")
                    # B
                    nt.links.new(sep.outputs['B'], emis.inputs['Color'])
                    try:
                        bpy.ops.object.bake(type='EMIT', margin=8)
                    except Exception as e:
                        self.report({'WARNING'}, f"{obj.name}: ORM B bake error: {e}")
            ...
            if img_norm:
                ...
                if nimg:
                    ...
                    try:
                        ...
                        self.report({'WARNING'}, f"{obj.name}: Normal bake error: {e}")
                        if s.ar_debug:
                            log(e)
        ...
        self.report({'INFO'}, f"Rebake completed for {len(sel)} object(s) (AO→glTF Occlusion).")
        return {'FINISHED'}
